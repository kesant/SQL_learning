...
SELECT id_genero ,count(id_genero)
FROM canciones
GROUP BY id_genero;
...
SELECT SUM(total)
FROM facturas;
-- En el sitio web de Musimundos, queremos informar la duración promedio en milisegundos de cada álbum. Obtiene una lista de todos los álbumes y su duración (milisegundos) promedio. Inserta el número del promedio que obtuviste para el álbum  
SELECT  AVG(milisegundos) AS duracionPromedio
FROM canciones;
